...
public class GifController {
    ...
    @ResponseBody //Tells thymeleaf that what is returned should be the http response body. No view rendered.
    ...
    public String addGif(Gif gif, @RequestParam MultipartFile file, RedirectAttributes redirectAttributes) { //@RequestParam Tells spring that value "file" will be passed in payload
        // TODO: Upload new GIF if data is valid
        gifService.save(gif, file);

        //Add flash message for success
        redirectAttributes.addFlashAttribute("flash", new FlashMessge("GIF Successfully Uploaded", FlashMessge.Status.SUCCESS));
    ...
    public String updateGif(@Valid Gif gif, @RequestParam MultipartFile file, BindingResult result, RedirectAttributes redirectAttributes) {
        ...
        if(result.hasErrors()) {
            ...
            redirectAttributes.addFlashAttribute("gif", gif); //Model data survives 1 redirect
        ...
        redirectAttributes.addFlashAttribute("flash", new FlashMessge("Gif successfully updated", FlashMessge.Status.SUCCESS));
    ...
    public String deleteGif(@PathVariable Long gifId, RedirectAttributes redirectAttributes) {
        ...
        redirectAttributes.addFlashAttribute("flash", new FlashMessge("GIf successfully deleted", FlashMessge.Status.SUCCESS));
    ...
    public String toggleFavorite(@PathVariable Long gifId, HttpServletRequest request) {
        ...
        return String.format("redirect:%s", request.getHeader("referer")); //request.getHeader() gets the last page user was on
                                                                                 //THIS IS A VULNERABILITY THOUGH...if compromised could
                                                                                 //send user to malicious site
    }
